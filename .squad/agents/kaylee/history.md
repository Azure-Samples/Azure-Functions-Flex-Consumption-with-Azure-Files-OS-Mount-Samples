...
- **2026-03-10 — Cross-sample consistency pass.** Applied the direct `Microsoft.Web/sites@2024-04-01` pattern to `ffmpeg-image-processing/infra/app/function.bicep`, replacing the broken AVM `web/site:0.15.1` module. Both samples now use identical function.bicep structure: direct resource with `functionAppConfig`, `siteConfig.appSettings` as name/value array, UserAssigned identity, and same output shape. Also brought `durable-text-analysis/README.md` up to parity with the ffmpeg README (How It Works, Testing, Troubleshooting table, file structure, post-deploy hook note). Upgraded `durable-text-analysis/scripts/post-up.sh` with health check, richer sample text files, and complete azd env variable loading. Both Bicep files validated with `az bicep build`.
